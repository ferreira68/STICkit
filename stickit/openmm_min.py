...
def _to_openmm_context(mol: ROMol, ff_name) -> Simulation:
    ...
    if have_antechamber():
        ...
        print("WARNING: Unable to assign partial charges with AmberTools; using charges from molecule")
        charged_mols = [offmol]
    else:
        print("WARNING: Unable to assign partial charges with AmberTools; falling back to Gasteiger from RDKit")
        RDKitToolkitWrapper().assign_partial_charges(offmol, partial_charge_method="gasteiger")
        charged_mols = [offmol]

    interchange = Interchange.from_smirnoff(off_ff, offtop, charge_from_molecules=charged_mols)
