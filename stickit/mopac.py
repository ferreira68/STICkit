...
def _write_mopac(mol, conf_id, charge, method, tempK, path):
    ...
    freq_keywords = (f"{method} CHARGE={charge} EPS=78.4 RSOLV=1.4 OLDGEO +\n"
...
def _parse_results_block(block: str) -> MopacCalcResult:
    res = MopacCalcResult()
    ...
    if m: res.gradient_norm = float(m.group(1))

    m = re.search(r'IONIZATION POTENTIAL\s*=\s*([-\d.]+)\s*EV', block)
    if m: res.ionization_potential_ev = float(m.group(1))

    m = re.search(r'HOMO LUMO ENERGIES.*?=\s*([-\d.]+)\s+([-\d.]+)', block)
    if m: res.homo_lumo_ev = [float(m.group(1)), float(m.group(2))]
    ...
    freqs = re.findall(r'\bFREQUENC(?:Y|IES)\b.*?([0-9.]+)', block)
    if freqs:
        # Extract numeric tails; tolerant of extra text on same line
        nums = re.findall(r'\bFREQUENCY\s+([0-9.]+)', block)
        if nums:
...
def parse_mopac_output(path_or_text: Union[str, Path]) -> Dict[str, Any]:
    ...
    for blk in _split_calculations(text):
        if re.search(r'CALCULATION RESULTS|CALCULATION\s', blk):
            calcs.append(_parse_results_block(blk).to_dict())
    ...
    for stic in sticset.stics:
        ...
        with tempfile.TemporaryDirectory() as td:
            for conf in stic.conformers:
                ...
                results = parsed.get('calculations')[-1]
                freqs = results['vibrational_frequencies_cm_inv']
                conf.n_imag_freq = sum(freq < 0 for freq in freqs)
                energy_record = {results.get('method', 'MOPAC'): results.get('heat_of_formation_kcal_mol', None)}
                conf.enthalpy = energy_record
                conf.free_energy = next(
                        r["gibbs_free_energy_kcal_mol"] for r in results["thermo"] if r["temperature_K"] == 298.0
                )
